...
const Home = () => {
  ...
  const [currentkenoTime, setCurrentkenoTime] = useState(0);
  const [currenspinTime, setCurrenspinTime] = useState(0);
  ...
  }, [kenoTime, spinTime, location.pathname]);
  ...
  const mappedSlipData = slips.map((slip) => {
    return {
  ...
  const handlePlaceBet = async () => {
    if (
      ...
      mappedSlipData.some((slip) => slip.gameType === "Keno") &&
      currentkenoTime <= 5
    ) {
      alert("Keno game is about to start, please wait");
      return;
    } else if (
      mappedSlipData.some((slip) => slip.gameType === "SpinAndWin") &&
      currenspinTime <= 5
    ) {
      alert("Spin game is about to start, please wait");
      return;
    } else {
      const bets = [];
      const games = [];
      slips.forEach((slip) => {
        ...
        bets.push({
          ...
          selection:
            slip.game === "win"
              ? [slip.gameType === "Keno" ? parseInt(slip.value) : slip.value]
              : slip.value,
          stake: slip.amount,
        });
      });
  ...
  return (
    <div className="relative">
      ...
      <div className="flex justify-between px-2 py-3 shadow-lg">
        ...
        <div className="flex flex-col items-end">
          ...
          <div className="flex items-center justify-end gap-2 ">
            <p className="me-5">
              <span className="text-green-500 ">Cashier:</span>{" "}
        ...
        <div className="flex flex-col items-center w-full gap-2 pb-3 shadow-lg h-fit shadow-gray-200">
          ...
          {slips.length > 0 ? (
            <>
              {slips.map((slip) => {
                return (
                  <div className="w-full flex flex-col text-[0.7rem] bg-slipGray p-0.5 rounded-md text-white">
                    <div className="flex items-start w-full gap-2">
                      ...
                      <div className="flex text-[13px] flex-col w-2/3 font-bold">
                        ...
                        <p className="f">
                          {slip.date + " "}
                          <span className="text-white">ID: {slip.gameId}</span>
                        </p>
                    ...
                    <div className="flex justify-end gap-1 pr-11">
                      To Win:
                      <p className="font-bold "> Br {slip.amount * slip.odd}</p>
                    </div>
                  </div>
                );
              })}
                ...
                {[
                  ...
                  return (
                    <div
                      key={data.num}
                      className={clsx(
                        `w-[60px] -space-y-2 px-1 cursor-pointer flex flex-col justify-center text-white h-[40px] items-center rounded-[3px]`,
